...
//TEST    private Boolean processed;
...
//TEST            System.out.println("is emileinfo");
        ...
        if(verifyUserId(info)&&!verifyStreetDetailInfo(info)&&!verifyZipcode(info)&&!verifyPhoneNumber(info)&&!verifyQQNum(info)&&!verifyEmile(info))
        {
 ...
 // TEST          processed=true;
            ...
            {//如果是直辖市，只在直辖市中考虑
                ...
                for(Province p:allProvince)
                {
                    ...
                    {
                        if(p.getProvinceName().equals(municilality[i]) && !p.getProvinceName().equals(provinceName))
                        {
            ...
            if(!districtName.equals("")&&!cityName.equals("")&&!provinceName.equals(""))
            {
                ...
                breakFor:for (Province p : provincesToMakebelieve)
                {
                    ...
                    for(City c:p.getCitiesGoverned())
                    {
                        ...
                        for(District d:c.getDistrictList())
                        {
                            if(addressLength==d.getAdderessLength())
                            {
                                ...
                                processedCustomerInfo=processedCustomerInfo.replace(provinceName,makebelieveProvinceName);
                                processedCustomerInfo=processedCustomerInfo.replace(cityName,makebelieveCityName);
                                processedCustomerInfo=processedCustomerInfo.replace(districtName,makebelieveDistrictName);
                                processedCustomerInfo=processedCustomerInfo.replace(provinceName.substring(0,provinceName.length()-1),makebelieveProvinceName.substring(0,makebelieveProvinceName.length()-1));
                                processedCustomerInfo=processedCustomerInfo.replace(cityName.substring(0,cityName.length()-1),makebelieveCityName.substring(0,makebelieveCityName.length()-1));
                                break breakFor;
                            }
                        }
                    }
                ...
                breakFor:for(Province p:provincesToMakebelieve)
                {
                    ...
                    for(City c:p.getCitiesGoverned())
                    {
                        ...
                        {//如果是直辖市用区级名代替市级名
                            ...
                            for(District d:c.getDistrictList())
                            {
                                if(addressLength==d.getAdderessLength()-3)
                                {//减3是“市辖区”三个字的长度
                                    ...
                                    processedCustomerInfo=processedCustomerInfo.replace(provinceName.substring(0,2),makebelieveProvinceName.substring(0,2));
                                    break breakFor;
                                }
                                else
                                {//没有合适的“区”，则还是用“市辖区”
            ...
            else if(provinceName.equals("")&&!cityName.equals("")&&!districtName.equals(""))
            {
                ...
                breakFor:for(Province p:provincesToMakebelieve)
                {
                    ...
                    for(City c:p.getCitiesGoverned())
                    {
                        ...
                        for(District d:c.getDistrictList())
                        {
                            if((d.getAdderessLength()-p.getProvinceName().length())==addressLength)
                            {
                                makebelieveCityName=c.getCityName();
                                makebelieveDistrictName=d.getDistrictName();
                                processedCustomerInfo=processedCustomerInfo.replace(cityName,makebelieveCityName);
            ...
            else if(provinceName.equals("")&&!cityName.equals("")&&districtName.equals(""))
            {
                ...
                breakFor:for(Province p:provincesToMakebelieve)
                {
                    ...
                    for(City c:p.getCitiesGoverned())
                    {
                        if(addressLength==(c.getAddressLength()-p.getProvinceName().length()))
                        {
                            makebelieveCityName=c.getCityName();
                            processedCustomerInfo=processedCustomerInfo.replace(cityName,makebelieveCityName);
                            break breakFor;
...
//        if(processed)
//        {
//            File file = new File("D:/Project_yl/CustomerInfoMask/src/masked-all-data.txt");
//            try {
//                if(!file.exists()) {
//                    file.createNewFile();
//                }
...
//        }
        ...
        {
            return processedStr+": Info length is out of contral！！！！！"+"  before:"+info.length()+"  after:"+processedStr.length();
        }
    ...
    public Boolean verifyUserId(String info)
    {
        ...
        if(info.contains("白T")||info.contains("长袖T")||info.contains("女儿")||info.contains("儿子")||info.contains("小孩")||info.contains("Ok")||info.contains("thanks")||info.contains("hello")||info.contains("HELLO")||info.contains("CM")||info.contains("3Q")||info.contains("EMS")||info.contains("ems")||info.contains("kg")||info.contains("KG")||info.contains("vip")||info.contains("ID")||info.contains("这款")||info.contains("nihao")||info.contains("T恤")||info.contains("t恤")||info.contains("身高")||info.contains("体重")||info.contains("https")|info.contains("http")||info.contains("码")||info.contains("快递")||info.contains("费")||info.contains("包邮")||info.contains("OK")||info.contains("ok")||info.contains("元")||info.contains("cm")||info.contains("公分")||info.contains("斤")||info.contains("米"))
        {result=false;}
        return  result;
    }
...
//           newCodeNumber+=random.nextInt(10);
     ...
     * @param number
     * @return maskedNumber
     * 传入判断过后的电话号码，并对其进行掩码（保留前三位），并返回结果
     */
  ...
  {
  Boolean result=false;
  if(provinceName.equals("上海市")||provinceName.equals("重庆市")||provinceName.equals("北京市")||provinceName.equals("天津市"))
    {
      result=true;
    }
  return result;
     ...
     * @param customerInfo
     * @return Boolean
     * 判断传入的query是否包含地址信息，包含省，或者不包含省但包含市信息，则判断为地址信息
     */
    private Boolean containsStreetInfo(String customerInfo) {
        ...
        if(customerInfo.equals("黑龙江")||customerInfo.equals("黑龙江省")||customerInfo.equals("新疆维吾尔自治区")||customerInfo.equals("西藏自治区")||customerInfo.equals("内蒙古自治区"))
        {
            result = true;
            ...
            for (Province p : allProvince) {
                if (customerInfo.contains(p.getProvinceName()) || customerInfo
                    ...
                    customerInfo=customerInfo.replace(provinceName.substring(0,provinceName.length()-1),"x");
                    //判断是否直辖市
                    if (isMunicipality(provinceName)) {
                        for (City c : p.getCitiesGoverned()) {
                            for (District d : c.getDistrictList()) {
...
//                            }
        ...
        if (!result) {
            for (Province p : allProvince) {
                for (City c : p.getCitiesGoverned()) {
                    if (customerInfo.contains(c.getCityName())&&!c.getCityName().equals("县")&&!c.getCityName().equals("区")&&!c.getCityName().equals("市")) {
                        result = true;
                        cityName = c.getCityName();
                        ...
                        if(index>(plength-1))
                        {
                            if(p.getProvinceName().substring(0,plength-1).equals(customerInfo.substring(index-plength+1,index)))
                            {
                                provinceName=p.getProvinceName();
                            }
                        }
     ...
     * @param info
     * @return Boolean
     * 在已经对字符串进行初步判断是否包含省级市级或市级信息后，处理可能包含地址信息的子串
     */
    ...
    private String maskStreatDetailInfo(String streetInfo)
    {
        ...
        //分割
            ...
            if(ch.equals("省")||ch.equals("市")||ch.equals("旗")||ch.equals("区")||ch.equals("县")||ch.equals("乡")||ch.equals("镇")||ch.equals("组")||ch.equals("村")||ch.equals("街")||ch.equals("弄")||ch.equals("路"))
            {
                if(ch.equals("街") && i<(streetInfo.length()-1)&& String.valueOf(streetInfo.charAt(i+1)).equals("道"))
                {
        ...
        for(int i=0;i<streetInfoSeg.length;i++) {
            ...
            //替换除了省市区外其余的地址信息
            String[] replaceInfo = {"花", "水", "池", "林", "雨", "润", "丹", "华", "宝", "荣","上","下","左","右","东","南","西","北","中"};
            String[] streetSign={"省","市","旗","县","村","乡","镇","组","街道","路"};
//去掉包含在匿名信息中的地址信息标识
            if(!makebelieveProvinceName.equals(""))
            {
                String provinceSign=makebelieveProvinceName.substring(makebelieveProvinceName.length()-1,makebelieveProvinceName.length());
            ...
            {
                String citySign=makebelieveCityName.substring(makebelieveCityName.length()-1,makebelieveCityName.length());
            ...
            {
                String districtSign=makebelieveDistrictName.substring(makebelieveDistrictName.length()-1,makebelieveDistrictName.length());
                for(int p=0;p<streetSign.length;p++)
                {
...
//TEST
//       try {
//           File file=new File("D:/Project_yl/CustomerInfoMask/src/mask-data3.txt");
